...
if (isset($_POST['submit'])) {
        ...
        if(empty($_SESSION['key_admin_infocours'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_admin_infocours'] = $csrftoken_text;
        }
        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_infocours'],session_id());
        if(hash_equals($csrftoken,$csrf))
        {

          unset($_SESSION['key_admin_infocours']);
          $_SESSION['key_admin_infocours']='';
        }
        else
        {
          session_destroy();
        ...
        if (empty($_POST['title'])) {
                $tool_content .= "<p class='caution_small'>$langNoCourseTitle<br />
                                  <a href='$urlPath'>$langAgain</a></p><br />";
        } else {
                ...
                $tool_content .= "<p class='success_small'>$langModifDone<br />
                        <a href='".$urlPath."'>$langBack</a></p><br />
