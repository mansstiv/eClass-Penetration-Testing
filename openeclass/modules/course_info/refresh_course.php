...
if(isset($submit)) {
		...
		if(empty($_SESSION['key_admin_refresh_course'])){
		$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
		$_SESSION['key_admin_refresh_course'] = $csrftoken_text;
		}
		$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_refresh_course'],session_id());
		if(hash_equals($csrftoken,$csrf))
		{

			unset($_SESSION['key_admin_refresh_course']);
			$_SESSION['key_admin_refresh_course']='';
		}
		else
		{
			session_destroy();
