...
if (isset($send_archive) and $_FILES['archiveZipped']['size'] > 0) {
    ...
    if(empty($_SESSION['key_admin_restore'])){
    	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    	$_SESSION['key_admin_restore'] = $csrftoken_text;
    }
    
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_restore'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {

      unset($_SESSION['key_admin_restore']);
      $_SESSION['key_admin_restore']='';
    }
    else
    {
      session_destroy();
    ...
    if(empty($_SESSION['key_url_path'])){
    	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    	$_SESSION['key_url_path'] = $csrftoken_text;
    }
    
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_url_path'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {

      unset($_SESSION['key_url_path']);
      $_SESSION['key_url_path']='';
    }
    else
    {
      session_destroy();
	...
	if (file_exists($pathToArchive)) {
	 ...
	 if(empty($_SESSION['key_admin_create_dir'])){
    	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    	$_SESSION['key_admin_create_dir'] = $csrftoken_text;
    }
    
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_create_dir'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {

      unset($_SESSION['key_admin_create_dir']);
      $_SESSION['key_admin_create_dir']='';
    }
    else
    {
      session_destroy();
...
elseif (isset($_POST['pathOf4path'])) {
	...
	if(empty($_SESSION['key_admin_real_restore'])){
    	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    	$_SESSION['key_admin_real_restore'] = $csrftoken_text;
    }
    
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_real_restore'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {

      unset($_SESSION['key_admin_real_restore']);
      $_SESSION['key_admin_real_restore']='';
    }
    else
    {
      session_destroy();
    ...
    $csrftoken2 = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_url_path'],session_id());
...
// -------------------------------------
	...
	<br /><br />
	<form action='".$urlPath."' method='post' name='sendZip' enctype='multipart/form-data'>
	...
	<input type='submit' name='send_path' value='".$langSend."' />
		<input type ='hidden'name='csrf' value = '{$csrftoken2}'>
...
function course_details($code, $lang, $title, $desc, $fac, $vis, $prof, $type) {
    ...
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_create_dir'],session_id());
		...
		echo "<form action='$urlPath' method='post'>";
...
function unpack_zip_show_files($zipfile)
{
	...
	while ($entries = readdir($handle)) {
		...
		if (is_dir($dirnameCourse.$entries))
			...
			<form action='".$urlPath."' method='post' name='restoreThis'>
			<ol>";
		...
		$retString .= "</ol><br /><input type='submit' value='$langRestore' name='pathOf4path' />    <input type ='hidden'name='csrf' value = '{$csrftoken}'></form></li>";
	}
	closedir($handle);
	$retString .= "</ol>\n";
