...
$titulaire_probable="$prenom $nom";
	...
	if(empty($_SESSION['key_create_course'])){
  $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
  $_SESSION['key_create_course'] = $csrftoken_text;
}


$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_create_course'],session_id());

$urlPath = htmlspecialchars($_SERVER ['PHP_SELF'], ENT_QUOTES, 'UTF-8');

$tool_content .= "<form method='post' name='createform' action='$urlPath' onsubmit=\"return checkrequired(this, 'intitule', 'titulaires');\"><input type ='hidden'name='csrf' value = '{$csrftoken}'>";
...
if (isset($_POST['create_course'])) {
		...
		if(hash_equals($csrftoken,$csrf))
		{
		    ...
		    unset($_SESSION['key_create_course']);
		    $_SESSION['key_create_course']='';
        ...
        $my_fp = fopen("../../courses/$repertoire/work/index.html", "w");
        ...
        $my_fp = fopen("../../courses/$repertoire/dropbox/index.html", "w");
