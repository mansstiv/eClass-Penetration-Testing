...
<div id=\"operations_container\">
  <ul id=\"opslist\">
    <li><a href=\"".$urlPath."?upload=1\">".$dropbox_lang['uploadFile']."</a></li>
  </ul>
</div>";
...
elseif(isset($_REQUEST['upload']) && $_REQUEST['upload'] == 1)
{
    ...
    if(empty($_SESSION['key_dropbox_submit'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_dropbox_submit'] = $csrftoken_text;
    }

    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_dropbox_submit'],session_id());
...
if (!isset($_GET['mailing']))  // RH: Mailing detail: no received files
{
	...
	{
        if(empty($_SESSION['key_dropbox_index_delete'])){
          $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
          $_SESSION['key_dropbox_index_delete'] = $csrftoken_text;
        }

        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_dropbox_index_delete'],session_id());

		$dr_lang_all = addslashes( $dropbox_lang["all"]);
		$tool_content .= "
      <th width='3' style='border: 1px solid #edecdf'>
        <a href='dropbox_submit.php?deleteReceived=all&amp;dropbox_unid=$dr_unid&token=$csrftoken' onClick=\"return confirmationall('".$dropbox_lang['all']."');\"><img src='../../images/delete.gif' title='$langDelete' /></a></th>";
	}
...
tCont9;
	...
	$tool_content .= "
        <a href=\"dropbox_submit.php?deleteReceived=".urlencode($w->id)."&amp;dropbox_unid=".urlencode($dropbox_unid)."&token=$csrftoken\" onClick='return confirmation(\"$w->title\");'>
...
$tool_content .= "
	...
	$tool_content .= "
        <th width='3' style='border: 1px solid #edecdf'>
            <a href='dropbox_submit.php?deleteSent=all&amp;dropbox_unid=".urlencode( $dropbox_unid).$mailingInUrl."&token=$csrftoken'
...
tCont12;
	...
	$tool_content .= "
	<a href=\"dropbox_submit.php?deleteSent=".urlencode($w->id)."&amp;dropbox_unid=".urlencode($dropbox_unid) . $mailingInUrl."&token=$csrftoken\"
