		...
		*/
		...
		if ($this->isOldWork) {
		   ...
		   //  $sql = "UPDATE `".$dropbox_cnf["fileTbl"]."`
		    ...
		    $sql = "UPDATE `".$dropbox_cnf["fileTbl"]."`
					SET filesize = '".addslashes($this->filesize)."', title = ?, description = ?
					, author = ?, lastUploadDate = '".addslashes($this->lastUploadDate)."'
					WHERE id= ?";

			// Perform query
			$stmt = mysqli_stmt_init($conn);
			mysqli_stmt_prepare($stmt, $sql);
			mysqli_stmt_bind_param($stmt, "sssi", addslashes($this->title), addslashes($this->description), addslashes($this->author), addslashes($this->id));
			...
			$sql="INSERT INTO `".$dropbox_cnf["fileTbl"]."` 
				(uploaderId, filename, filesize, title, description, author, uploadDate, lastUploadDate)
				VALUES ( ?, ?, '".addslashes($this->filesize)."', ?, ?, ?
						, '".addslashes($this->uploadDate)."', '".addslashes($this->lastUploadDate)."')";		
			// Perform query
			$stmt = mysqli_stmt_init($conn);
			mysqli_stmt_prepare($stmt, $sql);
			mysqli_stmt_bind_param($stmt, "issss", addslashes($this->uploaderId), addslashes($this->filename), addslashes($this->title), addslashes($this->description), addslashes($this->author));
			mysqli_stmt_execute($stmt);
			$result = mysqli_stmt_get_result($stmt);
