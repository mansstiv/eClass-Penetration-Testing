...
if (isset($_SESSION["dropbox_uniqueid"]) && isset($_GET["dropbox_unid"]) && $dropbox_unid == $_SESSION["dropbox_uniqueid"]) {
	...
	$mypath=$mypath."://".$_SERVER['HTTP_HOST'].dirname($urlPath)."/index.php";
...
if (isset($_GET['deleteSent']) || isset($_GET['deleteReceived'])) 
{
    if (isset($_GET['token'])){
...
if (isset($_POST["submitWork"]))
{
    ...
    {
        unset($_SESSION['key_dropbox_submit']);
        $_SESSION['key_dropbox_submit']='';
    }
    else
    {
        session_destroy();
	...
	if (!$error) {
		...
		if (!$error) {
			...
			if (!$error)
			{
				...
				$ext = get_file_extension($dropbox_filename);

				$ultrahidden = bin2hex(openssl_random_pseudo_bytes(30));
				$dropbox_filename = $ultrahidden.$dropbox_filename;
