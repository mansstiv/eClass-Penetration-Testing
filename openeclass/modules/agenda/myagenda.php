...
function get_agendaitems($query, $month, $year) {
	...
	while ($mycours = mysql_fetch_array($query))
	{
		...
		$connn = mysqli_connect("db", "root", "gSQw8OyTPD");
		...
		$mysqlQ = "SELECT * FROM agenda WHERE month(day)=? AND year(day)=?";

		mysqli_select_db($connn, $mycours[k]);
...
//		$result = db_query("SELECT * FROM agenda WHERE month(day)=? AND year(day)=?","$mycours[k]");
		...
		mysqli_stmt_prepare($stmt, $mysqlQ);
...
function display_monthcalendar($agendaitems, $month, $year, $weekdaynames, $monthName, $langToday) {
	...
	$backwardsURL = "$urlPath?month=".($month==1 ? 12 : $month-1)."&year=".($month==1 ? $year-1 : $year);
	$forewardsURL = "$urlPath?month=".($month==12 ? 1 : $month+1)."&year=".($month==12 ? $year+1 : $year);


	$backwardsURL=htmlspecialchars($backwardsURL, ENT_QUOTES, 'UTF-8');
	$forewardsURL=htmlspecialchars($forewardsURL, ENT_QUOTES, 'UTF-8');
