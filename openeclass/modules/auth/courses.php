...
if (isset($_POST["submit"])) {
    ...
    if(empty($_SESSION['key_auth_courses'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_auth_courses'] = $csrftoken_text;
    }

    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_auth_courses'],session_id());
    ...
    if(!$pageWasRefreshed ) 
    {
        ...
        {
            unset($_SESSION['key_auth_courses']);
            $_SESSION['key_auth_courses']='';
        }
        else
        {
		...
		if (isset($result))  {
			...
			while ($fac = mysql_fetch_array($result)) {
				...
				$tool_content .= "\n<td>&nbsp;<img src='../../images/arrow_blue.gif' />&nbsp;
					<a href='$urlPath?fc=$fac[id]'>" . htmlspecialchars($fac['name']) . "</a> <small><font color='#a33033'>($fac[code])</font></small>";
...
function collapsed_facultes_horiz($fc) {
    ...
    $urlPath = htmlspecialchars($_SERVER ['PHP_SELF'], ENT_QUOTES, 'UTF-8');
	$retString = "\n   <form name='depform' action='$urlPath' method='get'>\n";
