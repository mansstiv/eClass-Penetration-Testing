...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$tool_content .= $lang_pass_intro;
			...
			if(empty($_SESSION['key_pass_lost'])){
		
		$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
		$_SESSION['key_pass_lost'] = $csrftoken_text;
		
		}
		
		$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_pass_lost'],session_id());
	...
	$tool_content .= "<form method=\"post\" action=\"".$REQUEST_URI."\">
		...
		if(hash_equals($csrftoken,$csrf))
		{
			...
			unset($_SESSION['key_pass_lost']);
			$_SESSION['key_pass_lost']='';
		}
		else
		{
			session_destroy();
