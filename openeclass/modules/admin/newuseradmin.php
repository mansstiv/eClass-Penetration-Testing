...
if(empty($_SESSION['key_newuseradmin'])){
			$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
			$_SESSION['key_newuseradmin'] = $csrftoken_text;
	}
			$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_newuseradmin'],session_id());
...
if($submit) {
	...
	if (!$all_set) {
		$tool_content .= "<p class='caution_small'>$langEmptyFields</p>
			<br><br><p align='right'><a href='$urlPath'>$langAgain</a></p>";
	} elseif ($user_exist) {
		...
		$tool_content .= "<p class='caution_small'>$langEmailWrong.</p>
			...
			if(hash_equals($csrftoken,$csrf))
			{
				...
				unset($_SESSION['key_newuseradmin']);
				$_SESSION['key_newuseradmin']='';
			}
			else
			{
				session_destroy();
