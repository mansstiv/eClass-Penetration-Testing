...
if(empty($_SESSION['key_admin_announcement'])){
  $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
  $_SESSION['key_admin_announcement'] = $csrftoken_text;
}


$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_announcement'],session_id());
...
if (isset($_GET['delete'])) {
    ...
    if(hash_equals($csrftoken,$csrf))
    {
        ...
        unset($_SESSION['key_admin_announcement']);
        $_SESSION['key_admin_announcement']='';
...
if ($displayForm && (@$addAnnouce==1 || isset($modify))) {
        ...
        $tool_content .= "<form method='post' action='$urlPath?localize=$localize'>";
...
if ($displayAnnouncementList == true) {
        ...
        if (@$addAnnouce != 1) {
                ...
                <ul id='opslist'><li>";
                $tool_content .= "<a href='".$urlPath."?addAnnouce=1&amp;localize=$localize'>".$langAdminAddAnn."</a>";
        ...
        while ($myrow = mysql_fetch_array($result)) {
                ...
                &nbsp;&nbsp;
                <a href='$urlPathmodify=$myrow[id]&amp;localize=$localize'>
                <img src='../../template/classic/img/edit.gif' title='$langModify' style='vertical-align:middle;' />
                </a>&nbsp;
                <a href='$urlPath?delete=$myrow[id]&amp;localize=$localize' onClick='return confirmation();'>
