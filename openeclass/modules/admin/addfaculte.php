<style type="text/css" media="screen">	
	.button-edit {
	    background: url(../../template/classic/img/edit.gif) no-repeat;
        cursor:pointer;
        border: none;
    }
...
else if (isset($_POST['button-edit'])) {
	$a = 3;
}
...
if (isset($a)) {
	switch ($a) {
		case 1:
			$navigation[] = array("url" => "$urlPath", "name" => $langListFaculteActions);
...
if(empty($_SESSION['key_addfaculte'])){
    $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    $_SESSION['key_addfaculte'] = $csrftoken_text;
}

$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_addfaculte'],session_id());

// Check if button selected was for editing or deletion
if (isset($_POST['button-delete']) || isset($_POST['edit']) || isset($_POST['add'])) {
    ...
    if(!$pageWasRefreshed ) {
		...
		{
			unset($_SESSION['key_addfaculte']);
			$_SESSION['key_addfaculte']='';
		}
		else
		{
			session_destroy();
...
******************************************************************************/
	...
	<ul id='opslist'>
	<li><a href='$urlPath?a=1'>".$langAdd."</a></li>
	</ul>
	</div>";
...
if (!isset($a)) {
	...
	for ($j = 0; $j < mysql_num_rows($sql); $j++) {
		...
		<form action='$urlPath' method='post'>
			<input type ='hidden' name='c' value = '{$logs['id']}'>
			<input type='submit' name='button-delete' border='0' title='$langDelete' class='button-delete' value=''> 
			&nbsp;&nbsp;
			<input type='submit' name='button-edit' border='0' title='$langEdit' class='button-edit' value=''> 
...
elseif ($a == 1)  {
	...
	if (isset($add)) {
		...
		if (empty($codefaculte) or empty($faculte)) {
			...
			<a href=\"$urlPath?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check for greek letters
		elseif (!preg_match("/^[A-Z0-9a-z_-]+$/", $codefaculte)) {
			$tool_content .= "<p>".$langGreekCode."</p><br />";
			$tool_content .= "<center><p><a href=\"$urlPath?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		...
		$tool_content .= "<form method=\"post\" action=\"".$urlPath."?a=1\">";
		...
		<tr>
		<th>&nbsp;</th>
		<td><input type='submit' name='add' value='".$langAdd."' />
			<input type ='hidden'name='csrf' value = '{$csrftoken}'>
		</td>
		</tr>
		</tbody>
	...
	$tool_content .= "<br /><p align='right'><a href='$urlPath'>".$langBack."</a></p>";
}
// Delete faculty
elseif ($a == 2) {
        $c = intval($_POST['c']);
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		if (empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br>";
			$tool_content .= "<p align='right'><a href='$urlPath?a=3&c=$c'>$langReturnToEditFaculte</a></p>";
			}
		...
		$tool_content .= "<form method='post' action='$urlPath?a=3'>";
		$tool_content .= "<table width='99%' class='FormData'>
		<tbody>
		<tr>
		<th width='220'>&nbsp;</th>
