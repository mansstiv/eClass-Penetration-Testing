...
$local_head = $jscalendar->get_load_files_code();
    ...
    {
      if(empty($_SESSION['key_platform_Stats'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_platform_Stats'] = $csrftoken_text;
        }
        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_platform_Stats'],session_id());
        if(hash_equals($csrftoken,$csrf))
        {

          unset($_SESSION['key_platform_Stats']);
          $_SESSION['key_platform_Stats']='';
        }
        else
        {
          session_destroy();
