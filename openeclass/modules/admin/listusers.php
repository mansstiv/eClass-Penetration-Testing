	...
	if($countUser>0)
	{
		...
		if ($countUser >= $endList) { // Do not show navigation buttons if less than 30 users	
			$tool_content .= "<table class=\"FormData\" width=\"99%\" align=\"left\">
  			<thead><tr>
    			<td class=\"left\" width=\"20%\">
      			<form method=post action=\"$urlPath?numbList=begin\">
      			".keep_var()."
				...
				if (isset($_REQUEST['ord'])) {
	       				$tool_content .= "<form method=post action=\"$urlPath?startList=$startList&numbList=less&ord=$_REQUEST[ord]\">
	       				".keep_var()."
					<input type=submit value=\"$langPreced50<\" name=\"numbering\">
					</form>";
				} else {
			  		$tool_content .= "
      					<form method=post action=\"$urlPath?startList=$startList&numbList=less\">
      					".keep_var()."
        				<input type=submit value=\"$langPreced50<\" name=\"numbering\">
      					</form>";
				}
			...
			if (isset($_REQUEST['ord']))  {
	    			$tool_content .= "</td><td class=\"center\" width=\"20%\">
      				<form method=post action=\"$urlPath?startList=$startList&numbList=all&ord=$_REQUEST[ord]\">
      				".keep_var()."
				...
				$tool_content .= "</td><td class=\"center\" width=\"20%\">
      				<form method=post action=\"$urlPath?startList=$startList&numbList=all\">
      				".keep_var()."
        			<input type=submit value=\"$langAll\" name=numbering>
      				</form></td>
    				<td class=\"center\" width=\"20%\">";
			...
			if(!((($countUser-$startList) <= $endList) OR ($endList == $countUser))) // if end of list or complete listing, do not show "next" button
			{
				if (isset($_REQUEST['ord'])) { 
					$tool_content .= "
      					<form method=post action=\"$urlPath?startList=$startList&numbList=more&ord=$_REQUEST[ord]\">
      					".keep_var()."
        				<input type=submit value=\"$langFollow50>\" name=numbering>
      					</form>";
				} else {
	      				$tool_content .= "
      					<form method=post action=\"$urlPath?startList=$startList&numbList=more\">
      					".keep_var()."
        				<input type=submit value=\"$langFollow50>\" name=numbering>
      					</form>";
				}
			...
			if (isset($_REQUEST['ord'])) {
	  			...
	  			<form method=post action=\"$urlPath?numbList=final&ord=$_REQUEST[ord]\">
	  			".keep_var()."
	    			...
	    			$tool_content .= "</td><td class=\"right\" width=\"20%\">
      				<form method=post action=\"$urlPath?numbList=final\">
      				".keep_var()."
        			<input type=submit value=\"$langEnd>>\" name=numbering>
       				</form>
     				</td></tr></thead></table>";
		...
		*****************************************/
		...
		if (isset($numbering) and isset($_REQUEST['startList']) and isset($_REQUEST['numbList'])) {
			...
			<a href='$urlPath?ord=n$string&$str&startList=$_REQUEST[startList]&numbList=$_REQUEST[numbList]'>$langSurname</a>
			</th><th>
			<a href=\"$urlPath?ord=p$string&$str&startList=$_REQUEST[startList]&numbList=$_REQUEST[numbList]\">$langName</a>
			</th><th>
			<a href=\"$urlPath?ord=u$string&$str&startList=$_REQUEST[startList]&numbList=$_REQUEST[numbList]\">$langUsername</a></th><th scope=\"col\">$langEmail</th>
    			<th scope=\"col\">
			<a href=\"$urlPath?ord=s$string&$str&startList=$_REQUEST[startList]&numbList=$_REQUEST[numbList]\">$langProperty</a></th><th scope=\"col\">$langActions</th>
		...
		else
		{
			$tool_content .= "<th scope='col' colspan='2'>
				<a href='$urlPath?ord=n&$str'>$langSurname</a></th>
    			<th><a href='$urlPath?ord=p&$str'>$langName</a></th>
    			<th><a href='$urlPath?ord=u&$str'>$langUsername</a></th>
    			<th scope='col'>$langEmail</th>
    			<th scope='col'>
			<a href='$urlPath?ord=s&$str'>$langProperty</a></th>
