...
if (isset($_POST['submit']))  {
	   ...
	   if(empty($_SESSION['key_statuscours_admin'])){
	
		  $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
		  $_SESSION['key_statuscours_admin'] = $csrftoken_text;
	
	  }


	  $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_statuscours_admin'],session_id());

      if(hash_equals($csrftoken,$csrf))
      {

          //unset($_SESSION['key_statuscours_admin']); 
          //$_SESSION['key_statuscours_admin']='';
...
function reverseAll(cbList) {
	...
	$tool_content .= "<form action=".$urlPath."?c=".htmlspecialchars($_GET['c'])."".$searchurl." method='post'>";
	...
	$tool_content .= "<input type ='hidden'name='csrf' value = '{$csrftoken}'></form>";
