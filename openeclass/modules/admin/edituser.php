...
if(empty($_SESSION['key_admin_edit_user'])){
	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	$_SESSION['key_admin_edit_user'] = $csrftoken_text;
}


$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_edit_user'],session_id());
  ...
  <div id=\"operations_container\">
    <ul id=\"opslist\">";
      ...
      <li><a href='./listusers.php'>$langBack</a></li>";
		...
		$tool_content .= "
<form name='edituser' method='post' action='$urlPath'>
    ...
    <td colspan='2'>&nbsp;</td>
		...
		else
		{
			...
			else
			{
				...
				{
					unset($_SESSION['key_admin_edit_user']);
					$_SESSION['key_admin_edit_user']='';
				}
				else
				{
					session_destroy();
...
if (mysql_num_rows($username_check) > 1) {
  ...
  if (empty($fname) OR empty($lname) OR empty($username)) {
	...
	<p><a href='$urlPath'>$langAgain</a></p></td></tr></tbody></table><br /><br />";
