...
if (isset($submit))  {
  // Get faculte ID and faculte name for $faculte
      ...
      if(empty($_SESSION['key_admin_info_cours'])){
  $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
  $_SESSION['key_admin_info_cours'] = $csrftoken_text;
  }


  $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_admin_info_cours'],session_id());
      ...
      if(hash_equals($csrftoken,$csrf))
      {
          ...
          //unset($_SESSION['key_admin_info_cours']); 
          //$_SESSION['key_admin_info_cours']='';
...
// Display edit form for course basic information
  ...
  $urlPath = htmlspecialchars($_SERVER ['PHP_SELF'], ENT_QUOTES, 'UTF-8');
	...
	$tool_content .= "
  <form action=".$urlPath."?c=".htmlspecialchars($_GET['c'])."".$searchurl." method=\"post\">
