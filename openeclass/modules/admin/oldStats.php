...
{
    if(empty($_SESSION['key_old_stats'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_old_stats'] = $csrftoken_text;
    }
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_old_stats'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {

      unset($_SESSION['key_old_stats']);
      $_SESSION['key_old_stats']='';
    }
    else
    {
      session_destroy();
