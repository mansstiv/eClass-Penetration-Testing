...
$basetoolurl = htmlspecialchars($_SERVER ['PHP_SELF'], ENT_QUOTES, 'UTF-8');
 ...
 // Display Actions Toolbar
  $tool_content .= "
	  ...
	  <li><a href='newuseradmin.php$linkget'>$linkreg</a></li>
          <li><a href='$urlPath?show=closed$reqtype'>$langReqHaveClosed</a></li>
          <li><a href='$urlPath?show=rejected$reqtype'>$langReqHaveBlocked</a></li>
          <li><a href='$urlPath?show=accepted$reqtype'>$langReqHaveFinished</a></li>
        </ul>
      </div>";
...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=center>
			<a href='$urlPath?id=$req[rid]&show=closed$reqtype'>$langRestore</a></td>\n  </tr>";
		$k++;
		}
	}
	$tool_content .= "\n  </tbody>\n  </table>\n";
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=\"center\">
				...
				<small>".nice_format(date("Y-m-d", strtotime($req['date_closed'])))."</small></td>";
                	$tool_content .= "<td>".$req['comment']."</td>";
			$tool_content .= "<td align=center>
			<a href='$urlPath?id=$req[rid]&show=closed$reqtype'>$langRestore</a>
			</td></tr>";
			$k++;
		}
	}
...
// ------------------------------
	...
	case '2':
		...
		if(!empty($submit)) {
			...
			if(empty($_SESSION['key_listeq_deny'])){
					$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
					$_SESSION['key_listeq_deny'] = $csrftoken_text;
			}
			$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_listeq_deny'],session_id());
			if(hash_equals($csrftoken,$csrf))
			{

				unset($_SESSION['key_listeq_deny']);
				$_SESSION['key_listeq_deny']='';
			}
			else
			{
				session_destroy();
...
else
{
	...
	while ($req = mysql_fetch_array($sql)) {
		...
		$tool_content .= "<td align='center'>
		<a href='$urlPath?id=$req[rid]&amp;close=1$reqtype' onclick='return confirmation();'>$langClose</a><br />
		<a href='$urlPath?id=$req[rid]&amp;close=2$reqtype'>$langRejectRequest</a>";
...
if (!empty($show)) {
	$tool_content .= "<p align=\"right\"><a href=\"$urlPath\">$langBackRequests</a></p><br>";
}
