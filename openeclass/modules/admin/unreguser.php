...
if(empty($_SESSION['key_unreguser_admin'])){
    $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    $_SESSION['key_unreguser_admin'] = $csrftoken_text;
}

$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_unreguser_admin'],session_id());
...
if (!$_POST['submit']) {
    ...
    */
        $tool_content .=  "<form id=myForm action='unreguser.php' method='post'>";
        $tool_content .= "  
            <ul class=\"listBullet\">
                <li>
                    <input class='FormData_InputText' type=radio name='persoStatus' value='yes'>Ναι&nbsp;
                </li>
                <li>
                    <input class='FormData_InputText' type=radio name='persoStatus' value='no' checked>Οχι
                </li>
            </ul>
            <input style='margin-top:5px;' type='submit' name='submit' value='$langSubmit' />
    ...
    {
        unset($_SESSION['key_unreguser_admin']);
