...
if ($is_adminOfCourse){
	...
	if  (isset($_REQUEST['toolStatus']) ){
		...
		if(empty($_SESSION['key_course_tools_tool_status'])){
       	 	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        	$_SESSION['key_course_tools_tool_status'] = $csrftoken_text;
        }
        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_course_tools_tool_status'],session_id());
        if(hash_equals($csrftoken,$csrf))
        {

          unset($_SESSION['key_course_tools_tool_status']);
          $_SESSION['key_course_tools_tool_status']='';
        }
        else
        {
          session_destroy();
		...
		//get the state of the agenda tool and store it in a session var. It is used to insert or deletezzz
	...
	if (isset($delete)) {
	    ...
	    if(empty($_SESSION['key_course_tools_delete'])){
	        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	        $_SESSION['key_course_tools_delete'] = $csrftoken_text;
	    }

		    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_course_tools_delete'],session_id());
	...
	if(isset($submit) &&  @$action == 2) {
		...
		if(empty($_SESSION['key_course_tools_add_external'])){
	   	 	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	    	$_SESSION['key_course_tools_add_external'] = $csrftoken_text;
	    }
	    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_course_tools_add_external'],session_id());
	    if(hash_equals($csrftoken,$csrf))
	    {

	      unset($_SESSION['key_course_tools_add_external']);
	      $_SESSION['key_course_tools_add_external']='';
	    }
	    else
	    {
	      session_destroy();
		...
		if (($link == "http://") or ($link == "ftp://") or empty($link) or empty($name_link))  {
			$tool_content .= "<p class=\"caution_small\">$langInvalidLink<br /><a href=\"" . $urlPath . "?action=2\">$langHome</a></p><br />";
...
// -------------------------
	...
	if(isset($submit) &&  @$action == 1){
		...
		if(empty($_SESSION['key_submit_istoselida'])){
       	 	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        	$_SESSION['key_submit_istoselida'] = $csrftoken_text;
        }
        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_submit_istoselida'],session_id());
        if(hash_equals($csrftoken,$csrf))
        {

          unset($_SESSION['key_submit_istoselida']);
          $_SESSION['key_submit_istoselida']='';
        }
        else
        {
          session_destroy();
		...
		if (isset($file_name) and ($file_name != "") && ($file_size <= "$size") and ($link_name != "")) {
			...
			$tool_content .= "<p class=\"caution_small\">$langTooBig<br /><a href=\"" . $urlPath . "?action=1\">$langHome</a></p><br />";
			draw($tool_content, 2, 'course_tools');
		}	// else
		unset($action);
	}
...
if ($is_adminOfCourse && @$action == 1) {//upload html file
	...
	$tool_content .= "<form method=\"POST\" action=\"" . $urlPath . "?submit=yes&action=1\" enctype=\"multipart/form-data\">
	...
	<br>    <input type ='hidden'name='csrf' value = '{$csrftoken}'></form>";
	draw($tool_content, 2, 'course_tools');
	exit();
}
...
if ($is_adminOfCourse && @$action == 2) {//add external link
	...
	$tool_content .=  "<form method=\"post\" action=\"" . $urlPath . "?submit=yes&action=2\">
	<br>
	<table width=\"99%\" align='left' class='FormData'>
	<tbody>
	<tr>
	...
	</table>    <input type ='hidden'name='csrf' value = '{$csrftoken}'></form>";
	draw($tool_content, 2, 'course_tools');
	exit();
}
//---------------------------------------------------------
if ($is_adminOfCourse) {
	...
	<div id=\"operations_container\">
	  <ul id=\"opslist\">
	    <li><a href=\"" . $urlPath . "?action=1\">".$langUploadPage."</a></li>
	    <li><a href=\"" . $urlPath . "?action=2\">".$langAddExtLink."</a></li>
	  </ul>
	</div>";
	...
	$tool_content .= <<<tForm
<form name="courseTools" action="$urlPath" method="post" enctype="multipart/form-data">
  <br/>
	...
	if ($extToolsCount>0)  {
		...
		for ($i=0; $i <$extToolsCount; $i++) {
			...
			$tool_content .= "<td align='center'>
    			<a href=\"" . $urlPath . "?delete=" . $externalLinks[$i]['id']."&token=".$csrftoken."\" onClick=\"return confirmation('".addslashes($externalLinks[$i]['text'])."');\">
