...
if ($is_adminOfCourse and isset($_GET['assignment']) and isset($_GET['submission'])) {
    ...
    $assignment_my = intval($_GET['assignment']);
    $submission_my = intval ($_GET['submission']);

		$assign = get_assignment_details($assignment_my);
		$navigation[] = array("url"=>"work.php", "name"=>$langWorks);
		$navigation[] = array("url"=>"work.php?id=$_GET[assignment]", "name"=>$m['WorkView']);
		show_edit_form($assignment_my, $submission_my, $assign);
		draw($tool_content, 2);
} else {
