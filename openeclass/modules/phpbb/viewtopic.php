...
mysqli_stmt_bind_param($stmt, "ii",  $forum, $topic);
...
if ($paging and $total > $posts_per_page ) {
	...
	for($x = 0; $x < $total; $x += $posts_per_page) {
		...
		if($start && ($start == $x)) {
			...
			$tool_content .= "\n<a href=\"$chaaange?mode=viewtopic&amp;topic=$topic&amp;forum=$forum&amp;start=$x\">$times</a>";
		}
		$times++;
	}
	...
	if ( isset($start) && $start > 0 ) {
		...
		$tool_content .= "\n       <a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;start=$last_page\">$langPreviousPage</a>&nbsp;|";
	} else {
		$start = 0;
	}	
	if (($start + $posts_per_page) < $total) {
		...
		$tool_content .= "\n       <a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;start=$next_page\">$langNextPage</a>&nbsp;|";
	}
			$chaaange = htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8');

	$tool_content .= "&nbsp;<a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;all=true\">$langAllOfThem</a></span>
	</td>
	</tr>
	</thead>
	</table>";
	...
	if ($total > $posts_per_page) {	
		...
		&nbsp;<a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;start=0\">$langPages</a>
		</span>";
	}
...
do {
	...
	if ($is_adminOfCourse) { // course admin
        ...
        if(empty($_SESSION['key_editpost'])){
            $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
            $_SESSION['key_editpost'] = $csrftoken_text;
        }

        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_editpost'],session_id());
		...
		$tool_content .= "&nbsp;<a href='editpost.php?post_id=".$myrow["post_id"]."&amp;topic=$topic&amp;forum=$forum&amp;delete=on&amp;submit=yes&token=$csrftoken' onClick='return confirmation()'><img src='../../template/classic/img/delete.gif' title='$langDelete' alt='$langDelete' /></a>";
	}
...
cData;
	...
	$tool_content .= "&nbsp;<a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;all=true\">$langAllOfThem</a>
	</span>
	</td></tr></thead></table>";
} else {
	$tool_content .= "<table width=\"99%\"><thead>
	...
	if ($total > $posts_per_page) {	
		...
		$tool_content .= "&nbsp;<a href=\"$chaaange?topic=$topic&amp;forum=$forum&amp;start=0\">$langPages</a>";
        } else {
                $tool_content .= '&nbsp;';
        }
