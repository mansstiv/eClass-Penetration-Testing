...
$topic = intval($topic);
$forum = intval($forum);
if(empty($_SESSION['key_reply'])){
	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	$_SESSION['key_reply'] = $csrftoken_text;
}


$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_reply'],session_id());
...
if (isset($submit) && $submit) {
	...
	if ($this_post) {
		...
		mysqli_stmt_bind_param($stmt, "is",  $this_post, str_replace("'","", autoquote($message)));
