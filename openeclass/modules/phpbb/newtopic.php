...
if(empty($_SESSION['key_newtopic'])){
	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	$_SESSION['key_newtopic'] = $csrftoken_text;
}
...
if (isset($submit) && $submit) {
	if(hash_equals($csrftoken,$csrf))
	{
		...
		unset($_SESSION['key_newtopic']);
		$_SESSION['key_newtopic']='';
...
*/
	...
	mysqli_stmt_bind_param($stmt, "siisss", $temp_string, $uid, $forum, $time, $nom, $prenom);
...
/*
	...
	$stmt = mysqli_stmt_init($conn);
	mysqli_stmt_prepare($stmt, $sql);
	mysqli_stmt_bind_param($stmt, "iiissss", $topic_id, $forum, $uid, $time, $poster_ip, $nom, $prenom);
	mysqli_stmt_execute($stmt);
	...
	if (!$result = db_query($sql, $currentCourseID)) {
		...
		if ($post_id) {
			...
			*/
			...
			$temp_string = autoquote($message);
			$temp_string = str_replace("'","",$temp_string);

			// Perform query
			$stmt = mysqli_stmt_init($conn);
			mysqli_stmt_prepare($stmt, $sql);
			mysqli_stmt_bind_param($stmt, "is", $post_id, $temp_string);
			mysqli_stmt_execute($stmt);
			$result = mysqli_stmt_get_result($stmt);
