...
if(empty($_SESSION['key_editpost_form']))
{
    $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    $_SESSION['key_editpost_form'] = $csrftoken_text;
}

$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_editpost_form'],session_id());
...
if ($is_adminOfCourse) { // course admin 
	...
	if (isset($submit) && $submit) {
		...
		// Check if it comes from a delete call
		...
		// It comes from an edit form call so check for corresponding token
		else
		{
			if(hash_equals($csrftoken,$csrf))
			{
				unset($_SESSION['key_editpost_form']);
				$_SESSION['key_editpost_form']='';
			}
			else
			{
				session_destroy();
		...
		$chaaange = htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8');
			...
			$tool_content .= "<form action='$chaaange' method='post'>
			<table width='99%'>
