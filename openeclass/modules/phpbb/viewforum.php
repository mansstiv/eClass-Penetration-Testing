...
if (mysql_num_rows($result) > 0) { // topics found
	while($myrow = mysql_fetch_array($result)) {
		...
		if (isset($_GET['start']) and $_GET['start'] > 0) {
			$tool_content .= "<a href='$chaaange?forum=$forum&start=$_GET[start]&amp;topicnotify=$topic_link_notify&amp;topic_id=$myrow[topic_id]'>
			<img src='../../template/classic/img/announcements$topic_icon.gif' title='$langNotify'></img></a>";
		} else {
			$tool_content .= "<a href='$chaaange?forum=$forum&amp;topicnotify=$topic_link_notify&amp;topic_id=$myrow[topic_id]'>
			<img src='../../template/classic/img/announcements$topic_icon.gif' title='$langNotify'></img></a>";
		}
		$tool_content .= "</td></tr>";
	} // end of while
