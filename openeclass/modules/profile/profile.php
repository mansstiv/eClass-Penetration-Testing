...
if(empty($_SESSION['key_profile'])){
	$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	$_SESSION['key_profile'] = $csrftoken_text;
}

$csrftoken = hash_hmac('sha256','this is a random text:profile' . $_SESSION['key_profile'],session_id());
...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
	...
	if (empty($nom_form) OR empty($prenom_form) OR empty($username_form)) {
		header("location:". $chaaange."?msg=4");
		exit();
	}
	...
	// everything is ok
		...
		##[BEGIN personalisation modification]############
			if(hash_equals($csrftoken,$csrf))
			{
				...
				unset($_SESSION['key_profile']);
				$_SESSION['key_profile']='';
			}
			else
			{
				session_destroy();
		...
		$username_form = htmlspecialchars($username_form, ENT_QUOTES, 'UTF-8');
		$prenom_form = htmlspecialchars($prenom_form, ENT_QUOTES, 'UTF-8');
		$nom_form = htmlspecialchars($nom_form, ENT_QUOTES, 'UTF-8');
		$am_form = htmlspecialchars($am_form, ENT_QUOTES, 'UTF-8');
		...
		mysqli_select_db($conn, "eclass");

		$sql = "UPDATE user SET nom=?, prenom=?, username=?, email=?, am=?, 
		...
		mysqli_stmt_bind_param($stmt, "ssssii", $nom_form, $prenom_form, $username_form, 
												$email_form, $am_form, $_SESSION["uid"]);
		...
		if (mysqli_stmt_execute($stmt)) {
			...
			header("location:". $chaaange ."?msg=1");
			exit();
		}
...
if ((!isset($changePass)) || isset($_POST['submit'])) {
	...
	$tool_content .= "</ul></div>";
	$tool_content .= "<form method=\"post\" action=\"$sec?submit=yes\">	<input type ='hidden'name='csrf' value = '{$csrftoken}'>
<br/>
    <table width=\"99%\">
    <tbody><tr>
