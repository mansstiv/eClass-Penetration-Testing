...
if(empty($_SESSION['key_password'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_password'] = $csrftoken_text;
}



$csrftoken = hash_hmac('sha256','this is a random text:pass' . $_SESSION['key_password'],session_id());
...
if (isset($submit) && isset($changePass) && ($changePass == "do")) {
	...
	if(hash_equals($csrftoken,$csrf))
    {
	    ...
	    unset($_SESSION['key_password']);
	    $_SESSION['key_password']='';
	    if (empty($_SESSION['key_password']))
	            echo 'to kanei empty';
	...
	if($old_pass == $old_pass_db) {
		...
		$myUserId = $_SESSION["uid"];
		mysqli_stmt_bind_param($stmt, "si",$new_pass, $myUserId);
		mysqli_stmt_execute($stmt);
