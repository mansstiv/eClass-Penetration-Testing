...
if(empty($_SESSION['key_forum_admin_form'])){
    $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    $_SESSION['key_forum_admin_form'] = $csrftoken_text;
}

$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_forum_admin_form'],session_id());
...
if (isset($forumgoadd) || isset($forumgosave) || isset($forumcatadd) || isset($forumcatsave)) 
{
	...
	if(!$pageWasRefreshed ) 
	{
	    ...
	    {
	        unset($_SESSION['key_forum_admin_form']);
	        $_SESSION['key_forum_admin_form']='';
	    }
	    else
	    {
	        session_destroy();
...
if(isset($forumgo)) {
	...
	if ($result and mysql_num_rows($result) > 0) {
		$tool_content .= "<form action=\"$urlPath?forumgoadd=yes&ctg=$ctg&cat_id=$cat_id\" method=post>
        ...
        if(empty($_SESSION['key_forum_admin_delete'])){
            $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
            $_SESSION['key_forum_admin_delete'] = $csrftoken_text;
        }

        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_forum_admin_delete'],session_id());
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			$forum_moderator, $forum_type) = mysql_fetch_row($result)) {
				...
				$tool_content .= "\n<td align='center'>
				<a href='$urlPath?forumgoedit=yes&forum_id=$forum_id&ctg=$ctg&cat_id=$cat_id'>
				<img src='../../template/classic/img/edit.gif' title='$langModify' border='0'></img></a>&nbsp;
				<a href='$urlPath?forumgodel=yes&forum_id=$forum_id&cat_id=$cat_id&ctg=$ctg&ok=0&token=$csrftoken' onClick='return confirmation();'>
		...
		$tool_content .= "
		<form action=\"$urlPath?forumgoadd=yes&ctg=$ctg&cat_id=$cat_id\" method=post onsubmit=\"return checkrequired(this,'forum_name');\">
		...
		<div align=\"right\"><a href=\"$urlPath?forumadmin=yes\">$langBackCat</a></div>";
	}
	// forum go edit
	elseif(isset($forumgoedit)) {
		...
		$tool_content .= "
		<form action=\"$urlPath?forumgosave=yes&ctg=$ctg&cat_id=".@$cat_id."\" method=post onsubmit=\"return checkrequired(this,'forum_name');\">
	...
	elseif(isset($forumcatedit)) {
		...
		$tool_content .= "
  		<form action='$urlPath?forumcatsave=yes' method=post onsubmit=\"return checkrequired(this,'cat_title');\">
    		<input type=hidden name=cat_id value=$cat_id>
    		<table width=99% class=\"FormData\">
	...
	elseif (isset($forumcatsave)) {
		...
		$tool_content .= "\n<p class=\"success_small\">$langNameCatMod<br /><a href=\"$urlPath?forumadmin=yes\">$langBack</a></p>";
	}
	...
	elseif(isset($forumgosave)) {
		...
		<a href=\"$urlPath?forumgo=yes&cat_id=$cat_id&ctg=$ctg\">$langBack</a></p>";
	}
	...
	elseif(isset($forumcatadd)) {
		...
		<a href='$urlPath?forumadmin=yes'>$langBack</a></p>";
		}
	...
	elseif(isset($forumgodel)) {
		...
		$tool_content .= "\n<p class=\"success_small\">$langForumDelete<br />
			<a href=\"$urlPath?forumgo=yes&ctg=$ctg&cat_id=$cat_id\">$langBack</a></p>";
	} else {
		...
		$tool_content .= "<form action=\"$urlPath?forumadmin=yes\" method=post></td><tr><td>";
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			<a href='$urlPath?forumcatedit=yes&cat_id=$cat_id'>
			<img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;
			<a href='$urlPath?forumcatdel=yes&cat_id=$cat_id&ok=0&token=$csrftoken' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>
			<a href='$urlPath?forumcatnotify=$link_notify&cat_id=$cat_id'>	
		...
		$tool_content .= "</tbody></table></form><br/>
		<form action=\"$urlPath?forumcatadd=yes\" method=post onsubmit=\"return checkrequired(this,'catagories');\">
		<table width=99% class=\"FormData\" align=\"left\">
		<tbody><tr>
		<th width='220'>&nbsp;</th>
