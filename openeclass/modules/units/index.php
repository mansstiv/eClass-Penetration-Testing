...
foreach (array('previous', 'next') as $i) {
        ...
        if ($q and mysql_num_rows($q) > 0) {
                ...
                $link[$i] = "<a href='$chaaange?id=$q_id'>$arrow1$q_title$arrow2</a>";
        } else {
                $link[$i] = '&nbsp;';
        }
}
...
function actions($res_type, $resource_id, $status)
{
	...
	if (!$is_adminOfCourse) {
        ...
        if ($status != 'del') {
                $content = "<td width='3%'><a href='$chaaange?edit=$resource_id'>" .
                "<img src='../../template/classic/img/edit.gif' title='$langEdit' /></a></td>";
        } else {
                $content = '<td width="3%">&nbsp;</td>';
        }
        $content .= "<td width='3%'><a href='$chaaange?del=$resource_id'" .
	...
	if ($status != 'del') {
		if ($res_type == 'text' or $res_type == 'video' or $res_type == 'forum' or $res_type == 'topic') { 
			$content .= "<td width='3%'><a href='$chaaange?vis=$resource_id'>" .
        ...
        if ($resource_id != $GLOBALS['max_resource_id']) {
                $content .= "<td width='3%'><a href='$chaaange?down=$resource_id'>" .
                        "<img src='../../template/classic/img/down.gif' title='$langDown'></img></a></td>";
	} else {
		$content .= "<td width='3%'>&nbsp;</td>";
	}
        if (!$first) {
                $content .= "<td width='3%'><a href='$chaaange?up=$resource_id'>" .
