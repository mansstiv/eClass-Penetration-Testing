...
if(empty($_SESSION['key_muladd_admin'])){
$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
$_SESSION['key_muladd_admin'] = $csrftoken_text;
}


$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_muladd_admin'],session_id());
    ...
    $tool_content .= "
    <form method=\"post\" action=\"".$chaaange."\" enctype=\"multipart/form-data\">";
	$tool_content .= <<<tCont2
...
$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_muladd_admin'],session_id());
    ...
    if(hash_equals($csrftoken,$csrf))
    {
        ...
        //unset($_SESSION['key_muladd_admin']); 
        //$_SESSION['key_muladd_admin']='';
...
while ($myrow = mysql_fetch_array($result)) {
	...
	"<td><a href=\"$chaaange?add=$myrow[user_id]\">".
