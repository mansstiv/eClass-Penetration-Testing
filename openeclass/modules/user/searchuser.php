...
if($is_adminOfCourse) {
    ...
    if (isset($_GET['token'])){
        ...
        if(empty($_SESSION['key_user_change'])){
            $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
            $_SESSION['key_user_change'] = $csrftoken_text;
        }

        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_user_change'],session_id());
	...
	$tool_content .= "<form method='post' action='$chaaange'>";
	...
	if (!empty($query)) {
        ...
        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_user_change'],session_id());
			...
			if ($myrow["tutor"]=='0') {
				...
				<a href='$chaaange?giveTutor=yes&token=$csrftoken&user_id=$myrow[user_id]&$s' title='$langGiveTutor'>$langAdd</a></td>";
			} else {
				$tool_content .= "<td class=\"highlight\" align='center'>$langTutor<br>
				<a href='$chaaange?removeTutor=yes&token=$csrftoken&user_id=$myrow[user_id]&$s' title='$langRemoveRight'>$langRemove</a></td>";
			}
			// admin right
			if ($myrow["user_id"]!=$_SESSION["uid"]) {
				if ($myrow["statut"]=='1') {
					$tool_content .= "<td class='highlight' align='center'>$langAdministrator<br>
					<a href='$chaaange?removeAdmin=yes&token=$csrftoken&user_id=$myrow[user_id]&$s' title='$langRemoveRight'>$langRemove</a></td>";
				} else {
					$tool_content .= "<td valign='top' align='center' class='add_user'>
					<a href='$chaaange?giveAdmin=yes&token=$csrftoken&user_id=$myrow[user_id]&$s' title='$langGiveAdmin'>$langAdd</a></td>";
				}
			} else {
				if ($myrow["statut"]=='1') {
					...
					$tool_content .= "<td valign=\"top\" align='center'>
					<a href='$chaaange?giveAdmin=yes&token=$csrftoken&user_id=$myrow[user_id]&$s'>$langGiveAdmin</a></td>";
				}
			}
				$tool_content .= "<td valign=\"top\" align='center'>";
				$alert_uname = $myrow['prenom'] . " " . $myrow['nom'];
				$tool_content .= "<a href='$chaaange?unregister=yes&token=$csrftoken&user_id=$myrow[user_id]&$s' onClick=\"return confirmation('".addslashes($alert_uname)."');\">
