...
if (isset($_GET['giveTutor']) || isset($_GET['removeTutor']) || isset($_GET['giveAdmin']) || isset($_GET['removeAdmin']) || isset($_GET['unregister'])) {
    if (isset($_GET['token'])){

        if(empty($_SESSION['key_user'])){
            $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
            $_SESSION['key_user'] = $csrftoken_text;
        }

        $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_user'],session_id());
...
if ($countUser >= $endList) {
   ...
   <tr>
     <td valign='bottom' align='left' width='20%'>
       <form method='post' action='$chaaange?numbList=begin'>
    ...
    if ($startList!=0) {
        $tool_content .= "
       <form method='post' action='$chaaange?startList=$startList&amp;numbList=less'>
    ...
    $tool_content .= "
     ...
     <td valign='bottom' align='center' width='20%'>
       <form method='post' action='$chaaange?startList=$startList&amp;numbList=all'>
    ...
    if (!((($countUser-$startList)<=$endList) OR ($endList==2000))) {
        $tool_content .= "<form method='post' action='$chaaange?startList=$startList&amp;numbList=more'>
    ...
    $tool_content .= "
     ...
     <td valign='bottom' width='20%'>
       <div align='right'>
       <form method='post' action='$chaaange?numbList=final'>
...
while ($myrow = mysql_fetch_array($result)) {
        ...
        if(isset($status) && ($status["$currentCourseID"]=='1' OR $status["$currentCourseID"]=='2')) {
                // tutor right
                if ($myrow['tutor'] == '0') {
                        $tool_content .= "<td valign='top' align='center' class='add_user'><a href='$chaaange?giveTutor=$myrow[user_id]&token=$csrftoken' title='$langGiveTutor'>$langAdd</a></td>";
                } else {
                        $tool_content .= "<td class='highlight' align='center'>$langTutor<br /><a href='$chaaange?removeTutor=$myrow[user_id]&token=$csrftoken' title='$langRemoveRight'>$langRemove</a></td>";
                }

                // admin right
                if ($myrow['user_id'] != $_SESSION["uid"]) {
                        if ($myrow['statut']=='1') {
                                $tool_content .= "<td class='highlight' align='center'>$langAdministrator<br /><a href='$chaaange?removeAdmin=$myrow[user_id]&token=$csrftoken' title='$langRemoveRight'>$langRemove</a></td>";
                        } else {
                                $tool_content .= "<td valign='top' align='center' class='add_user'><a href='$chaaange?giveAdmin=$myrow[user_id]&token=$csrftoken' title='$langGiveAdmin'>$langAdd</a></td>";
                        }
                } else {
                        if ($myrow['statut']=='1') {
                                $tool_content .= "<td valign='top' class='highlight' align='center' title='$langAdmR'><b>$langAdministrator</b></td>";
                        } else {
                                $tool_content .= "<td valign='top' align='center'><a href='$chaaange?giveAdmin=$myrow[user_id]&token=$csrftoken'>$langGiveAdmin</a></td>";
                        }
                }
                $tool_content .= "<td valign='top' align='center'>";
                $alert_uname = $myrow['prenom'] . " " . $myrow['nom'];
                $tool_content .= "<a href='$chaaange?unregister=$myrow[user_id]&token=$csrftoken' onClick=\"return confirmation('".addslashes($alert_uname)."');\"><img src='../../template/classic/img/delete.gif' title='$langDelete' /></a>";
        }   // admin only
