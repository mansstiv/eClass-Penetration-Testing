...
if (isset($add)) {
	...
	if (isset($_GET['token'])){
	    ...
	    if(empty($_SESSION['key_add_user_to_lesson'])){
	        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
	        $_SESSION['key_add_user_to_lesson'] = $csrftoken_text;
	    }

	    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_add_user_to_lesson'],session_id());
	    ...
	    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_add_user_to_lesson'],session_id());
			...
			while ($myrow = mysql_fetch_array($result)) {
				...
				$tool_content .= "<td align=\"right\">$i.</td><td>$myrow[prenom]</td>
      				...
      				<td align=\"center\">
				<a href=\"$chaaange?add=$myrow[user_id]&token=$csrftoken\">$langRegister</a></td></tr>\n";
				$i++;
			}
			$tool_content .= "</tbody>";
			$tool_content .= "</table>";
