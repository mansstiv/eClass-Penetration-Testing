...
include('../../include/action.php');
  ...
  if(empty($_SESSION['key_stat_favourite'])){
    $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
    $_SESSION['key_stat_favourite'] = $csrftoken_text;
    }
    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_stat_favourite'],session_id());
    if(hash_equals($csrftoken,$csrf))
    {


      unset($_SESSION['key_stat_favourite']);
      $_SESSION['key_stat_favourite']='';
    }
    else
    {
          session_destroy();
