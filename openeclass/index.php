...
if (isset($_SESSION['shib_uname'])) { // authenticate via shibboleth
	...
	if(!empty($submit)) {
		if (isset($submit))
		{
			if(empty($_SESSION['key_logged_out_content'])){
				$csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
				$_SESSION['key_logged_out_content'] = $csrftoken_text;
			}

			$csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_logged_out_content'],session_id());

			if(hash_equals($csrftoken,$csrf))
			{
				unset($_SESSION['key_logged_out_content']);
				$_SESSION['key_logged_out_content']='';
			}
			else
			{
				session_destroy();
