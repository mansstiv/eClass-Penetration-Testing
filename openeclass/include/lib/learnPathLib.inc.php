...
function commentBox($type, $mode)
{
    ...
    if ( $mode == DISPLAY_ || $dsp == TRUE )
    {
        ...
        if ( empty($currentComment) )
        {
            ...
            {
                $output .= '' . "\n"
                .    '<a href="' . $urlPath . '?cmd=update' . $col_name . '">' . "\n"
                .    $langAdd . '</a>' . "\n"
                ;
            }
        }
        else
        {
            ...
            if ( $is_adminOfCourse )
            {
                $output .= '&nbsp;&nbsp;&nbsp;<a href="' . $urlPath . '?cmd=update' . $col_name . '">' . "\n"
                .    '<img src="../../template/classic/img/edit.gif" alt="' . $langModify . '" title="'.$langModify.'" />'
                .    '</a>' . "\n"
                .    '<a href="' . $urlPath.'?cmd=del' . $col_name . '" '
...
function nameBox($type, $mode, $formlabel = FALSE)
{
    ...
    if ( $mode == DISPLAY_ || $dsp == true )
    {
        ...
        if ( $is_adminOfCourse )
            $output .= '&nbsp;&nbsp;&nbsp;<a href="' . $urlPath . '?cmd=updateName">'
            .    '<img src="../../template/classic/img/edit.gif" alt="' . $langModify . '" title="' . $langModify . '" />'
            .    '</a>' . "\n";
        //$output .= '</strong>'."\n\n";
    }
...
function display_my_exercises($dialogBox, $style)
{
    ...
    $output .= '    <form method="POST" name="addmodule" action="' . $urlPath . '?cmdglobal=add">'."\n";
...
function display_my_documents($dialogBox, $style)
{
    ...
    and we can't go to a parent dir */
    {
      ...
      <td width="10" class="right"><a href="' . $urlPath . '?openDir=' . $cmdParentDir . '"><small>' . $langUp . '</small></a></td>';
    }
        $output .= '
    </tr>
    </thead>
    ...
    if ( $fileList )
    {
		...
		$dspFileName = htmlspecialchars($fileList['filename'][$fileKey]);
            ...
            elseif ($fileList['type'][$fileKey] == A_DIRECTORY)
            {
                ...
                $size        = '&nbsp;';
                $date        = '&nbsp;';
                $urlFileName = $urlPath . '?openDir=' . $cmdFileName;
            }
