			...
			if(file_exists($module_ini_dir = getcwd() . "/module.ini.php")) {
				...
				if (display_activation_link($module_id)) {
					...
					if (visible_module($module_id)) {
						...
						$mod_activation = "<a class='deactivate_module' href='$urlPath?eclass_module_id=$module_id&amp;hide=0&token=$csrftoken'>($langDeactivate)</a>";
					} else {
						$message = $langActivate;
						$mod_activation = "<a class='activate_module' href='$urlPath?eclass_module_id=$module_id&amp;hide=1&token=$csrftoken'>($langActivate)</a>";
					}
				}
			}
		}
 ...
 */
function lang_selections() {
	// Filter url for xss attack
	$urlPath = htmlspecialchars($_SERVER ['PHP_SELF'], ENT_QUOTES, 'UTF-8');
	$html = '<form name="langform" action="' . $urlPath . '" method="get" >';
