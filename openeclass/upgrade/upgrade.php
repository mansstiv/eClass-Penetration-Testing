...
{
    if(empty($_SESSION['key_upgrade_form'])){
        $csrftoken_text = md5(openssl_random_pseudo_bytes(32,$cstrong));
        $_SESSION['key_upgrade_form'] = $csrftoken_text;
    }

    $csrftoken = hash_hmac('sha256','this is a random text:newtopic' . $_SESSION['key_upgrade_form'],session_id());

    if(hash_equals($csrftoken,$csrf))
    {
        unset($_SESSION['key_upgrade_form']);
        $_SESSION['key_upgrade_form']='';
    }
    else
    {
        session_destroy();
